...
impl Client {
    pub async fn author_books(&self, author_id: i64, page: Page) -> Result<Vec<BookSummary>> {
        let data = self
            .execute::<AuthorBooks>(author_books::Variables {
                author_id,
                limit: page.limit,
                offset: page.offset,
            })
    ...
    pub async fn series_books(&self, series_id: i64, page: Page) -> Result<Vec<SeriesEntry>> {
        let data = self
            .execute::<SeriesBooks>(series_books::Variables {
                series_id,
                limit: page.limit,
                offset: page.offset,
            })
    ...
    pub async fn list_books(&self, list_id: i64, page: Page) -> Result<Vec<ListEntry>> {
        let data = self
            .execute::<ListBooks>(list_books::Variables {
                list_id,
                limit: page.limit,
                offset: page.offset,
            })
    ...
    pub async fn book_editions(&self, book_id: i64, page: Page) -> Result<Vec<Edition>> {
        let data = self
            .execute::<BookEditions>(book_editions::Variables {
                book_id,
                limit: page.limit,
                offset: page.offset,
            })

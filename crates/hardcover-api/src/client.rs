...
use crate::model::{
    Book, BookIdentifier, Contributor, ResolvedBook, ResolvedBy, SearchHit, SearchResults,
    SearchType, SeriesMembership, User,
};
use crate::queries::{
    book_by_id, book_id_by_isbn, book_id_by_slug, me, search, BookById, BookIdByIsbn, BookIdBySlug,
    Me, Search,
};
...
impl Client {
    ...
    pub(crate) async fn execute<Q: GraphQLQuery>(
        ...
        let parsed: Response<Q::ResponseData> = serde_json::from_value(raw)
            .map_err(|e| Error::Upstream(format!("unexpected response shape: {e}")))?;
        ...
        parsed
            .data
            .ok_or_else(|| Error::Upstream("response had no data".into()))
    }
    ...
    pub async fn search(
        ...
        let out = data
            .search
            .ok_or_else(|| Error::Upstream("search returned no payload".into()))?;
        ...
        let hits: Vec<SearchHit> = results
            ...
            .map(|hits| {
                hits.iter()
                    .filter_map(|h| hit_from_document(h.get("document")?))
                    .collect()
            })
            .unwrap_or_default();
        Ok(SearchResults {
    ...
    pub async fn resolve_book(&self, ident: &BookIdentifier) -> Result<ResolvedBook> {
        match ident {
            BookIdentifier::Id(id) => Ok(ResolvedBook {
                id: *id,
                resolved_by: ResolvedBy::Id,
            }),
            BookIdentifier::Slug(slug) => {
                let data = self
                    .execute::<BookIdBySlug>(book_id_by_slug::Variables { slug: slug.clone() })
                    .await?;
                let id = data
                    .books
                    .first()
                    .map(|b| b.id)
                    .ok_or_else(|| Error::NotFound(format!("book with slug {slug}")))?;
                Ok(ResolvedBook {
                    id,
            ...
            BookIdentifier::Isbn(isbn) => {
                let data = self
                    .execute::<BookIdByIsbn>(book_id_by_isbn::Variables { isbn: isbn.clone() })
                    .await?;
                let id = data
                    .editions
                    .first()
                    .map(|e| e.book_id)
                    .ok_or_else(|| Error::NotFound(format!("edition with ISBN {isbn}")))?;
                Ok(ResolvedBook {
                    id,
                    resolved_by: ResolvedBy::Isbn,
                })
            }
        }
    }
    ...
    pub async fn book(&self, id: i64) -> Result<Book> {
        let data = self
            .execute::<BookById>(book_by_id::Variables { id })
            .await?;
        let b = data
            .books_by_pk
        ...
        Ok(Book {
            ...
            series: b
                ...
                .map(|(position, s)| SeriesMembership {
                    id: s.id,
                    slug: s.slug,
                    name: s.name,
                    position,

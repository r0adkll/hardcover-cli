...
async fn lists_series_books_with_positions() {
    ...
    respond(
        &s,
        serde_json::json!({"variables": {"series_id": 6572}}),
        "series_books_page.json",
    )
    .await;
...
async fn lists_a_books_editions() {
    ...
    respond(
        ...
        "book_editions_page.json",
    )
    .await;

    let editions = client(&s)
        .book_editions(
            1,
            Page {
                limit: 3,
...
async fn lists_a_lists_books() {
    ...
    respond(
        &s,
        serde_json::json!({"variables": {"list_id": 301791}}),
        "list_books_page.json",
    )
    .await;

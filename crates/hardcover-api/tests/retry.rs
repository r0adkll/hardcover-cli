...
async fn retries_after_429_honouring_retry_after() {
    ...
    let c = Client::builder("test-token").base_url(s.uri()).retry(RetryPolicy { max_attempts: 3, ..RetryPolicy::default() }).build();
...
async fn gives_up_after_max_attempts() {
    ...
    let c = Client::builder("test-token").base_url(s.uri()).retry(RetryPolicy { max_attempts: 2, ..RetryPolicy::default() }).build();

    let err = c.me().await.unwrap_err();

    assert!(matches!(err, Error::RateLimited { retry_after_secs: Some(0) }), "{err:?}");
...
async fn retry_can_be_disabled() {
    ...
    let c = Client::builder("test-token").base_url(s.uri()).retry(RetryPolicy::none()).build();

    assert!(matches!(c.me().await.unwrap_err(), Error::RateLimited { .. }));
